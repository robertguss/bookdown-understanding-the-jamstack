...
To begin, you will need to download the [course repo](https://github.com/robertguss/howtocode-understanding-the-jamstack) and use the `start/` folder located within `Eleventy-Membership`. This repo contains the theme for our site which will allow us to hit the ground running. The theme itself was provided by Memberstack as just a `.zip` of html, css and js files. I then took all of those files and integrated them with Eleventy to make our lives easier and so you can become familiar with a very popular static site generator.
...
The terminal should display various url's to access the site.
...
It is a fairly simple site with some placeholder content and two pricing tables at the very bottom. Take a couple of mins to click around on some of the navigation links to see what the various pages look like. There is a login form, a sign up form and two membership areas one for basic memberships and one for premium.

What we are going to be using Memberstack for is handling our user login, authentication, sessions and payments. Essentially Memberstack will handle all of the heavy backend lifting, whereas we just have to provide the front-end content for our members.

## Integrating Memberstack

Before we can begin using [Memberstack](https://www.memberstack.com/), you need to sign up for a free account. Once you have signed up and are logged in you will be presented with the following screen.
...
Memberstack gives us a nice checklist we need to follow in order to setup our new site. Click on the "Create a membership" button on the Setup Checklist.

![Memberstack Create a membership](images/eleventy-membership/memberstack_create_a_membership.png)

On the next screen click on the "New Membership" button. This is where we will setup our two memberships, "basic" & "premium". Let's start with the basic plan first.
...
3. Enter $49 for the price. As this is the price listed on the pricing tables of our site.
4. Select "Monthly" for the billing frequency
5. Free Trial, Setup Fee & Collect taxes can all be toggled off for now.

Now click on the "Hide Content" button in the "Members-only content" section. In this section we are going to make sure that user who do not have access to either membership plan cannot access the content.

1. Enter "Basic Plan" for the content name.
2. Enter `basic` for the "Hide pages & folders" field. This will hide all of the basic plan content from users who do
   not have a basic plan.
3. Enter `login` for the "Access denied page." This will redirect users who try access the basic account page who are
   not logged in or who are not paying subscribers.
...
This tells memberstack to redirect the user who has a "basic plan" membership to the basic account page after signing up and each time they login.
...
2. Select the "Require Payment" button.
3. Enter $499 for the price. As this is the price listed on the pricing tables of our site.
...
3. Enter `login` for the "Access denied page." This will redirect users who try access the premium account page who are not logged in or who are not paying subscribers.
4. Click the "Create" button at the bottom.

![Memberstack Premium Page Hide Content](images/eleventy-membership/memberstack_premium_page_hide_content.png)

Make sure that both the "Basic Plan" & "Premium Plan" are toggled on. This is because our premium members get access to both basic and premium content.
...
This tells memberstack to redirect the user who has a "premium plan" membership to the premium account page after signing up and each time they login.
...
Now that we have both of memberships setup and configured, we need to integrate them on our site so that our users can signup for them.

Underneath the "Membership Plans" section of our dashboard are our two membership plans. To the right of those are buttons called "Signup link" this is the code snippet we need to link our memberships on our site to Memberstack. Click on the "Signup link" for the Basic plan first.
...
```html
<a
  data-ms-membership="5fd8ca138868e80004829790"
  href="signup"
...
```html
<a
  data-ms-membership="5fd8cfb14b5ead000426cdfe"
  href="signup"
...
Copy the code snippet in there and then open add the snippet just above the closing `</head>` tag in `src/_includes/layouts/base.html`. The file should now look like this:
...
<link
  ...
  <link
    href="/assets/images/Freebie-Webclip.png"
    rel="apple-touch-icon">

  <!-- Memberstack -->
  <script
    src="https://api.memberstack.io/static/memberstack.js?custom"
    data-memberstack-id="751ee1afe565f84062a614b9a5d554a3">
  </script>
</head>
```
...
Next we need to integrate Memberstack with out Signup Form. On the Memberstack main dashboard, click on the "Add a Login form" under the "More Features" section.

![Memberstack Add Login Form](images/eleventy-membership/memberstack_add_login_form.png)

You will be taken to the "Login" form section, but we need to setup the "Signup" form first. So click on the "Signup" tab at the top.
...
Next let's setup our login form.
...
Now that we have our site setup with Memberstack, we need to deploy it to a live url for testing. We will be deploy with [Surge](https://surge.sh/).
...
Next we can deploy our site with this simply command.
...
Press enter and in a few seconds your site will be live. Head on over to the domain surge provides and open it your browser.
...
After you have updated the script with your surge url. Run the `deploy` command to make sure everything is working correctly.
...
First, let's try to access the `/basic/acount` page which is only reserved for users who have a basic membership. On your live surge site, go to `https://faulty-hate.surge.sh/basic/account` replacing the domain with whichever one Surge provided you.

You should be redirected to the `login` page

Now try to access the premium account by going to `/premium/account/`. For example
`https://faulty-hate.surge.sh/premium/account/`.
...
After a few seconds, you should be redirected to `/basic/account` which is the basic account membership area. So far so good! Memberstack handles all of the session and authentication for us, so we are currently logged in as a basic member.
...
Now that we are signed in, we need a way for our users to be able to sign out. We need to add a log out button to the nav of our `/basic/account` page right next to the "Change Plan" button.
...
You should still be logged in, so navigate to `/basic/account` and you should see our new "Log Out" button at the top. When you click it, you should be redirected to the home page. Try to access `/basic/account` again and you should be redirected to the home page.

### Premium Membership Signup

Now that we have confirmed our basic membership is working, let's test our the premium membership.

On the home page, scroll to the pricing tables at the bottom and click on the premium membership. Fill out the form and you should be presented with the Premium payment form.
...
This page will also need a log out button, but you already know how to do that, so if you want one on this page as well, by all means go for it.

## Connecting Stripe for payments

_**This next section is entirely optional and is only useful if you are launching a live membership site. Currently there is no way to test payments taken from Memberstack with Stripe while in the demo/test mode. Your membership site must be live in order the payments to show up in Stripe.**_

All of our testing thus far has been in test mode. If you noticed when we actually tried to signup for these plans, we didn't have to enter in any actual credit card information. We can connect Stripe so we can actually receive real money from real customers.

First, you will need to setup a [free Stripe account](https://stripe.com/). After that, head back over to the Memberstack dashboard.
...
On this screen click the big button to connect to stripe and once you are logged into your stripe account you should see a screen like this.
...
Well that wraps up this project. Just to recap, we used Memberstack to create a membership Jamstack site with two membership plans "basic" & "premium". Within memberstack we were able to designate which portion of the site basic and premium members have access to and also redirect users who do not have the appropriate access. We also learned how to integrate Stripe if we decide to launch a live membership site.

We also gained some experience with the Eleventy static site generator. This SSG is quite popular and in growing ever more so in popularity. It is quite simple to use and has a lot of features built in by default. It provides a variety of templating languages and other features that make getting Jamstack site up and running really quickly.

For hosting and deployment, we used Surge. Surge is a very simple NPM package that allows us to publish static sites with just a single command.